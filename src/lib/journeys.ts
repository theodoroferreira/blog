...
export function getAllTags() {
  ...
  for (const file of files) {
    const fullPath = path.join(JOURNEYS_DIR, file);
...
export function getJourneys() {
  ...
  return allJourneysData.sort((a, b) => {
    if (a.date < b.date) {
      return 1;
...
export async function getJourneyData(id: string) {
  const fullPath = path.join(JOURNEYS_DIR, `${id}.md`);
  const fileContents = fs.readFileSync(fullPath, "utf-8");
  const matterResult = matter(fileContents);
